  ...
  async function fetchSnippet() {
    try {
      ...
      if (res.ok) {
        ...
        setCode(data.code);
        setLanguages(data.language.split(",").map((l) => l.trim()));
        setTags(data.tags ? data.tags.map((t) => t.tag.name) : []);
      } else {
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <Link
              href={`/dashboard/${id}`}
              className="inline-flex items-center gap-1 text-sm text-[#8b949e] hover:text-[#d2a8ff] mb-3 transition-colors"
            >
            ...
            <h1 className="text-xl font-bold text-[#e6edf3]">Editar snippet</h1>
          </div>
        </div>
        ...
        <div className="bg-[#161b22] border border-[#21262d] rounded-xl p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-[#c9d1d9] mb-2">Título</label>
              <input
                type="text"
                value={title}
            ...
            <div>
              <label className="block text-sm font-medium text-[#c9d1d9] mb-2">
                Descrição <span className="text-[#6e7681] font-normal">(opcional)</span>
              </label>
              <input
                type="text"
                value={description}
            ...
            <div>
              <label className="block text-sm font-medium text-[#c9d1d9] mb-2">
                Tags <span className="text-[#6e7681] font-normal">(pressione Enter para adicionar)</span>
              </label>
